...
module Knapsack {
    ...
    function weightsLT'(s: seq<Obj>, weight: int): seq<Obj>
    requires |s|>0
    decreases |s|
    reads s
    {
    ...
    function a3(os: seq<Obj>, cap: int): int
    {
        //forall i :: 0 <= i <= cap ==>
        0
    }

    function a3'(os: seq<Obj>, curW: int): int
    decreases os
    reads os
    requires forall o :: o in os ==> o.weight <= curW 
    requires |os| > 0
    {
        if M.max(a3'(weightsLT'(os, os[0].weight), os[0].weight)+os[0].value, a3'(weightsLT'(os, os[1].weight), os[1].weight)+os[1].value) 
            == a3'(weightsLT'(os, os[0].weight), os[0].weight)+os[0].value then a3'(weightsLT'(os, os[0].weight), os[0].weight)+os[0].value
            else a3'(weightsLT'(os, os[1].weight), os[1].weight)+os[1].value
    }
