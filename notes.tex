...
\begin{algorithmic}[1]
	...
	\STATE Find some $\cal{P}$ \COMMENT{Can use $DFS$ or $BFS$ for this}
	\WHILE {$\exists \cal{P}$}
		\STATE $c(\cal{P}) \gets \min_{\forall e \in \cal{P}} (c_e-f_e)$ \COMMENT{Revisit garbled characters.}
...
In \ref{alg:ford_fulk}, we augment by increasing flow along forward edges by $c(\cal{P})$, and decreasing flow by $c(\cal{P})$ for back-edges. The proof-of-correctness relies on the max-flow = min-cut theorem, \ref{thm:maxflowmincut}. 
...
It takes $O(|V|)$ time to update $G^f$, and it takes $O(|E|)$ time, assuming $|E| \geq |V|-1$ since $G$ may have multiple cycles, by running $DFS$ or $BFS$ to find \cal{P} in the loop. Augmenting $f$ in the loop is also $O(|V|$, thus the running time is dominated by $O(|E|)$. 

We must take into account that there's $\leq C$ rounds we iterate through, thus the final running time pseudo-polynomial at:
