...
\begin{claim}
	$\exists$ an algorithm to find some alternating path $\in G$ in $O(|V|+|E|)$ using a modified $BFS$ algorithm.
\end{claim}
...
\begin{proof}
	Let $X$ be a set of uncovered vertices, i.e. members of endpoints in $E - M$.
	
	We can direct the edges for each matching edge $(u,v) \in M$, s.t. $\overrightarrow{uv}$.
\end{proof}
