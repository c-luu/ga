...
\begin{proof}
	\textbf{Claim 1}:\\
	Let $X$ be the set of edges from $V_1$ to $V_2$. We claim that $M \subseteq X$. 
	
	Let $A \subseteq V_1$ and $B \subseteq V_2$  be the set of uncovered edges.
	
	We can find an alternating path by running $BFS$ on any of $v \in A$, visiting any of $v$'s neighbors that are incident to some $m \in M$. We traverse until we reach some $u \in B$, indicating we've found an alternating path.
\end{proof}
