...
\label{prop:cal_p}	
	...
	$\mathcal{P}$ is a path between $(s,t) \in G^f$ with some available capacity. I.e., $\mathcal{P}$ is an $f$-augmenting \textit{st-path} $\in G^f$. 
\end{definition}
...
\textbf{Solution 1}
\begin{claim}
	New max-flow can be computed in $O(n+m)$ by simply constructing $G^f$ and applying \ref{lem:aug_path}.
\end{claim}

We are given a network-graph $G$, and the maximal flows, $f'=\{f_e\}_{e \in E}$. $f'$ can be thought of as our flow-graph, that has already been calculated for us. We have everything needed to compute $G^f$, which \textit{should be} done in $O(n+m)$ time per lectures.

Once $G^f$ is constructed, we can run $DFS$ on $G^f$ to see if $\exists$ a path from $s$ to $t$- but then what?

\newpage

\textbf{Solution 2}
\begin{claim}
Since we are given the maximal flow integer values for all $f_e \in E$, we could run a non-looping variant of $FordFulkerson$ to compute the new maximum flow in $O(m)$.
\end{claim}

\begin{proof}
	Let $C = $ the size of max flow \textit{prior} to increasing the capacity of some $e$ by 1. After increasing $e$'s capacity by 1, we construct $G^{f^*}$ using $f^*$, taking $O(n)$ time. We can do this while still preserving the pre-condition that available capacities must exist before constructing $G^{f^*}$ because we just increased some $e$'s capacity. We then check \ref{lem:aug_path} using $DFS$, taking $O(m)$ time. If $\mathcal{P}$ was found, we augment $f^*$ along this path in $O(n)$ time, otherwise return $f^*$.
