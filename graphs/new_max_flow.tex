...
\label{def:f_star}
	...
	Total flow for all edges, $e \in E$, s.t. $|f|$ is maximal.
\end{definition}
...
\begin{claim}
Since we are given the maximal flow integer values $\forall f_e \in E$, we could run $FordFulkerson$ to compute the new maximum flow in $O(m)$ in one round instead of $C$ rounds.
\end{claim}
