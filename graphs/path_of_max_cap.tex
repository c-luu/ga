...
We can accomplish this by modifying $Dijkstra$'s algorithm for computing single-shortest paths. The key modification is modifying the original $relax$ algorithm to \ref{alg:rel_new}, and returning a path using $P[t]$ starting at $t$ and back-tracking the parent links until $s$ is reached. 

The priority queue, $Q$, pops vertices in descending order of their $max\_c$ priority values. I.e., if $v$ is popped from $Q$, $max\_c[v]$ is the maximum value in $Q - \{v\}$. W.l.o.g., \ref{alg:rel_new} implicitly pushes $v$ to $Q$ with priority $max\_c[v]$ if $(u,v)$ is relaxed.
...
For initial conditions, assume $\forall v \in V - \{s\}, v \in Q$ and $max\_c[v] = -\infty$, and set $S$ contains $s$, with $max\_c[s] = \infty$. 
